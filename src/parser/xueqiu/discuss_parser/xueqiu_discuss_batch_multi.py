    ...
    for f in file_list:
        data_list.append(pd.read_csv(f, header=0,
                                     names=[u'create_at', u'desc', u'id', u'rcreate_at', u'rdesc', u'rid', u'rtitle',
                                            u'ruid', u'screen_sname', u'uid'], dtype={u'id': str, u'uid': str}, encoding='utf-8'))
...
def discuss_batch(discuss_df, xq_discuss_parser):
    # 对讨论数据做分词并提取股票列表，使用多进程解析器
    result_df = xq_discuss_parser.run(discuss_df)
    ...
    del xq_discuss_parser, result_df, transform_res_df, transform_res_grouped
    gc.collect()
    return result
...
if __name__ == '__main__':
    ...
    while tmp_time >= start_time:
        ...
        # 调用多进程解析器
        result = discuss_batch(discuss_df, xq_discuss_parser)

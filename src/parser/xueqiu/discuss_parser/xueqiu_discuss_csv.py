...
if __name__ == '__main__':
    ...
    logging.logger.info('length of result dataframe: %s' % len(result_dataframe))

    # 数据库存储
    # engine_sqlite = data_source.GetDataEngine('XAVIER_SQLITE')
    engine_mysql = data_source.GetDataEngine("XAVIER")

    result_dataframe.to_sql('history_discuss_stock_filter_test', engine_mysql, if_exists='replace', index=False)
